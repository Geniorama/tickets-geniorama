...
import { linkVaultToProject, unlinkVaultFromProject } from "@/actions/vault.actions";
...
interface ProjectVaultPanelProps {
  projectId: string;
  linkedEntries: VaultEntry[];       // entradas ya vinculadas, visibles para este usuario
  availableEntries: VaultEntry[];    // entradas accesibles que aún no están vinculadas
  canManage: boolean;                // staff + admin
}
...
export function ProjectVaultPanel({
  ...
  function handleLink() {
    ...
    startTransition(async () => {
      const result = await linkVaultToProject(projectId, selectedId);
  ...
  function handleUnlink(vaultEntryId: string) {
    startTransition(async () => {
      const result = await unlinkVaultFromProject(projectId, vaultEntryId);
      if (result?.error) setError(result.error);
    });
  }
    ...
    <div style={{
      ...
      {linkedEntries.length === 0 ? (
        <p style={{ fontSize: "0.8125rem", color: "var(--app-text-muted)", marginBottom: canManage ? "0.875rem" : 0 }}>
          No hay accesos vinculados a este proyecto.
        </p>
      ) : (
