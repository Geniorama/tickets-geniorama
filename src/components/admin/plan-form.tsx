...
export function PlanForm({ companies }: { companies: Company[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [type, setType] = useState<"BOLSA_HORAS" | "SOPORTE_MENSUAL">("BOLSA_HORAS");
  const [expiresType, setExpiresType] = useState<"none" | "duration" | "date">("none");
