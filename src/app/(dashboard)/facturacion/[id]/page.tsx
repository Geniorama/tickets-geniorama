...
export default async function BillingItemPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [comments, attachments, etiquetas] = await Promise.all([
  ...
  return (
    <div>
      ...
      {/* La cabecera dice de un vistazo las tres cosas por las que se entra:
          qué es, en qué estado está y cuánto falta por cobrar. */}
      <div
        className="mb-5"
        style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1.5rem", flexWrap: "wrap" }}
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1.25rem" }}>
          <div style={{ textAlign: "right" }}>
            <p style={{ fontSize: "1.5rem", fontWeight: 700, fontVariantNumeric: "tabular-nums", margin: 0, color: falta === 0 ? "#16a34a" : "#dc2626" }}>
              {formatAmount(falta === 0 ? cobro.amount : falta)}
            </p>
            <p style={{ fontSize: "0.75rem", color: "var(--app-text-muted)", margin: 0 }}>
              {falta === 0 ? "cobrado" : `por cobrar de ${formatAmount(cobro.amount)}`}
            </p>
          </div>
          {canEdit && (
            <Link
              href={`/facturacion/${cobro.id}/editar`}
              style={{
                display: "inline-flex", alignItems: "center", gap: "0.4rem",
                fontSize: "0.8125rem", padding: "0.5rem 0.95rem", borderRadius: "0.5rem",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4" style={{ alignItems: "start" }}>
        ...
        {/* La columna estrecha: lo que se consulta de vez en cuando y no se
            mira mientras se cuadra el dinero. */}
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          {/* Las fechas y la nota: se consultan, no se operan. */}
          <div
            style={{
              backgroundColor: "var(--app-card-bg)", border: "1px solid var(--app-border)",
              borderRadius: "0.75rem", padding: "1.25rem",
          ...
          >
            <p style={{ fontSize: "0.8125rem", fontWeight: 600, color: "var(--app-text-muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "0.5rem" }}>
              Ficha
            </p>
            <Sello etiqueta="Toca facturar el" fecha={cobro.dueDate} />
            ...
            <p style={{ fontSize: "0.75rem", color: "var(--app-text-muted)", marginTop: "0.2rem" }}>
              Creado por {cobro.createdBy.name}
            </p>
            {cobro.notes && (
              <p style={{ fontSize: "0.8125rem", color: "var(--app-nav-text)", whiteSpace: "pre-wrap", marginTop: "0.6rem", paddingTop: "0.7rem", borderTop: "1px solid var(--app-border)" }}>
                {cobro.notes}
              </p>
            )}
          </div>
