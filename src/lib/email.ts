...
/**
 * Código de 6 dígitos para vincular un número de WhatsApp a la cuenta.
 *
 * Va por correo a propósito: es lo que impide que alguien que conozca el email
 * de un cliente ate el bot a su propio teléfono. Si ya había un número
 * vinculado se menciona, para que el titular note un cambio que no pidió.
 */
export async function sendWhatsappCodeEmail(
  to: Recipient,
  code: string,
  newPhone: string,
  ...
  await client.sendMail({
    from: FROM,
    to: [{ email_address: { address: to.email, name: to.name } }],
    subject: `${code} es tu código para vincular WhatsApp`,
    htmlbody: `
      <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; color: #1f2937;">
        <div style="background: #4f46e5; padding: 24px 32px; border-radius: 8px 8px 0 0;">
          <h1 style="margin: 0; color: #ffffff; font-size: 20px;">Geniorama Tickets</h1>
        ...
        <div style="background: #ffffff; padding: 32px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 8px 8px;">
          <p style="margin-top: 0;">Hola <strong>${to.name}</strong>,</p>
          <p>Alguien pidió vincular el número <strong>${newPhone}</strong> a tu cuenta para gestionar tickets por WhatsApp.</p>
          <div style="text-align: center; margin: 32px 0;">
            <div style="display: inline-block; background: #f3f4f6; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px 32px; font-size: 32px; font-weight: bold; letter-spacing: 8px; color: #1f2937;">
              ${code}
            </div>
          </div>
          <p style="text-align: center; color: #6b7280; font-size: 13px;">El código caduca en <strong>10 minutos</strong>.</p>
          ${
            currentPhone
              ? `<p style="background: #fef3c7; border-left: 3px solid #f59e0b; padding: 12px 16px; font-size: 13px; color: #92400e;">
                   Tu cuenta ya tiene vinculado el número <strong>${currentPhone}</strong>. Si continúas, se reemplazará por el nuevo.
                 </p>`
              : ""
          }
          <p style="color: #6b7280; font-size: 13px;">
            <strong>Si no fuiste tú, ignora este correo</strong> y avisa a tu agente: sin el código, ese número no puede ver nada de tu cuenta.
          </p>
        </div>
      </div>
    `,
